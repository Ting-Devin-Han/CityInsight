...
class SpaceNet(Dataset):
    def __init__(self, args, data_path , transform = None, mode = 'Training',plane = False):
        ...
        images = sorted(glob(os.path.join(data_path, "SN1_buildings_train_AOI_1_Rio_3band/3band/*.tif")))
        masks = sorted(glob(os.path.join(data_path, "mask/multi_buildingMaskImages/*.tif")))
